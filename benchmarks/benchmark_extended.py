#!/usr/bin/env python3
"""
ParetoMol Extended Benchmark — larger molecule counts using ChEMBL data.
Runs only the new sizes (2500, 5000, 10000) and appends to existing results.json.
"""
...
CHEMBL_CSV = SCRIPT_DIR / "chembl_10k.csv"
RESULTS_JSON = SCRIPT_DIR / "results.json"

NEW_SIZES = [2500, 5000, 10000]
REPEATS = 3
...
def load_smiles_pool():
    ...
    print(f"Loaded {len(smiles)} SMILES from ChEMBL dataset")
    return smiles


def make_csv_subset(smiles_pool: list, n: int) -> str:
...
def benchmark_via_file_upload(page, csv_text: str, n: int) -> dict:
    ...
    try:
        ...
        return {"n": n, "loadTimeMs": round(load_time, 1), "moleculesLoaded": metrics["moleculesLoaded"]}
...
def run():
    ...
    for r in existing["results"]:
        if r.get("mean_ms", -1) > 0:
            print(f"{r['n']:>10} {r['mean_ms']/1000:>10.2f} {r.get('std_ms', 0):>10.0f}")
    print("=" * 60)
